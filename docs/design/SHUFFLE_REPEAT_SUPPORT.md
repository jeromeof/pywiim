...
#### Spotify
- ✅ **Album playback**: On-demand content, queue-based → Should support shuffle/repeat
- ❌ **Radio/Mix**: Algorithmic streaming, not queue-based → May NOT support controls
- **Why**: Spotify Radio is generated by Spotify's algorithm, not a fixed queue

#### Amazon Music
- ✅ **Album playback**: User's queue → Should support controls
- ❌ **Station**: Algorithm-generated → May NOT support controls

#### TuneIn
- ❌ **Live radio**: Streaming broadcast → No shuffle/repeat
- ✅? **Podcast**: On-demand, episode list → Might support controls

#### Apple Music (via AirPlay)
- ❌ **All content**: iOS device controls playback, not the WiiM device
...
**Goal**: Finally nail down shuffle/repeat support with real-world testing! 🎯
